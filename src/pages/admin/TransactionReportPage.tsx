...
import { buildInvoicePrintPayload } from '@/lib/invoicePrint'
...
export default function TransactionReportPage() {
  ...
  const printFromDetail = async (d: TxDetail) => {
    const payload = buildInvoicePrintPayload(d)
    const res = await window.api.printer.printInvoice(payload)
    if (res.success) toast.success('Sent to printer')
    else toast.error(res.error || 'Failed to print')
  }

  const reprintInvoice = async () => {
    if (!detail) return
    setReprinting(true)
    try {
      await printFromDetail(detail)
    } catch (e: any) {
  ...
  const quickReprint = async (id: string) => {
    setRowPrintingId(id)
    try {
      const res = await window.api?.reports.transactionDetail(id)
      if (!res?.success) { toast.error(res?.error || 'Failed to load invoice'); return }
      await printFromDetail(res.data as TxDetail)
    } catch (e: any) {
      toast.error(e?.message || 'Failed to print')
    } finally {
          ...
          <div className="bg-surface-800 rounded-xl border border-surface-600 w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-surface-700">
              ...
              <div className="flex items-center gap-2">
                ...
                {detail && (
                  <button onClick={reprintInvoice} disabled={reprinting}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface-600 hover:bg-surface-500 text-slate-200 text-xs font-medium disabled:opacity-50">
                    <Printer className="w-3.5 h-3.5" /> {reprinting ? 'Printing…' : 'Reprint'}
                  </button>
                )}
