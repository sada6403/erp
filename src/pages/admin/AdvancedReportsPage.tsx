...
export default function AdvancedReportsPage() {
  ...
  const metadata = {
    ...
    'Report': reportTabs.find(t => t.key === active)?.label || active,
  ...
  const exportExcel = async () => {
    ...
    await window.api?.reports?.exportExcel?.({
      ...
      sheets: [
        ...
        { name: reportTabs.find(t => t.key === active)?.label || active, rows: cleanRows(filteredRows) },
  ...
  const exportPdf = async () => {
    setExporting(true)
    await window.api?.reports?.exportPdf?.({ filename: `${active}-${today}` })
    setExporting(false)
  }
