...
export function registerCouponHandlers() {
  ...
  ipcMain.handle('coupons:create', async (_e, payload: Record<string, unknown>) => {
    try {
  ...
  ipcMain.handle('coupons:list', (_e, filters: Record<string, unknown> = {}) => {
    try {
      const perms = currentPermissions()
  ...
  ipcMain.handle('coupons:get', (_e, idOrCode: string) => {
    try {
      const perms = currentPermissions()
      if (!perms.all && !perms.coupons) {
        return { success: false, error: 'You do not have permission to view coupons' }
  ...
  ipcMain.handle('coupons:validate', (_e, code: string) => {
    try {
      const db = getDb()
  ...
  ipcMain.handle('coupons:void', async (_e, id: string, reason?: string) => {
    try {
      const perms = currentPermissions()
  ...
  ipcMain.handle('coupons:reports', (_e, filters: Record<string, unknown> = {}) => {
    try {
      const perms = currentPermissions()
